...
function initMap(day) {
	...
	if (day > numberofdays) {
		...
		// Diaplay path on map
		createwaypointsfromtransitcalculation( masterData[day - 1], day);
	}
...
function createwaypointsfromtransitcalculation( datadaywise, day) {
    console.log('createwaypointsfromtransitcalculation  called');

	waypts = [];
	var arrayindex = day - 1;

	for (var i = 0; i < datadaywise.length; i++) {
    console.log('datadaywise ' +JSON.stringify(datadaywise[i].lat));

		var id = "day" + day + "placecb" + i ;
		var checkBox = document.getElementById(id);
        place=[parseFloat(datadaywise[i].lat),parseFloat(datadaywise[i].lng)];
        waypts.push(place);
	...
	setTimeout(function() {
		...
		displayRoute( waypts, day);

		}, 500);

}

function displayRoute(waypts,	day) {
console.log('displayRoute called');
//var polygon=L.polygon(waypts).addTo(map);
var marker1 = L.marker([51.5, -0.09]).addTo(map);
var polygon1= L.polygon([[50.8427501,4.3515499],[50.8427501, 4.3515499],[50.8427501,4.3515499]]).addTo(map);

}


//This function plots Individual day
function displayRouteOld(directionsService, directionsDisplay, start, end, waypts,	day) {
